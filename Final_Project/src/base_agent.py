...
class TD3BaseAgent(ABC):
		...
		for episode in range(self.total_episode):
			...
			for t in range(10000):
				...
				if terminates or truncates:
					...
					print(
						'Step: {}\tEpisode: {}\tLength: {:3d}\tTotal reward: {:.2f}\tProgress: {:.3f}\tCheckpoint: {:1d}'
						.format(self.total_time_step, episode+1, t, total_reward, info['progress'], info['checkpoint']))
			...
			if (episode+1) % self.eval_interval == 0:
				...
				self.save(os.path.join(self.writer.log_dir, f"model_{self.total_time_step}_{round(avg_score, 3)}.pth"))
	...
	def evaluate(self):
		...
		for episode in range(self.eval_episode):
			total_reward = 0
			state, infos = self.env.reset(train=False)
			for t in range(10000):
				action = self.decide_agent_actions(state)
				next_state, reward, terminates, truncates, info = self.env.step(action, test=True)
				total_reward += reward
				state = next_state
				if terminates or truncates:
					print(
						'Episode: {}\tLength: {:3d}\tTotal reward: {:.2f}\tProgress: {:.3f}\tCheckpoint: {:1d}'
						.format(episode+1, t, total_reward, info['progress'], info['checkpoint']))
					all_rewards.append(total_reward)
					all_progress.append(info['progress'])
					break

		avg = sum(all_rewards) / self.eval_episode
		avg_progress = sum(all_progress) / self.eval_episode
		print(f"average score: {avg}, average all_progress: {avg_progress}")
		print("==============================================")
		return avg
